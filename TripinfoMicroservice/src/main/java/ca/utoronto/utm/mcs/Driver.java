...
public class Driver extends Endpoint{
    ...
    public void handleGet(HttpExchange r) throws IOException, JSONException {
        ...
        try {
            // Get all Trips
            FindIterable<Document> allTrips = this.dao.retrieveDocumentsDriver(uid);
            ArrayList<JSONObject> fullTrips = new ArrayList<>();
            JSONObject current;

            for(Document doc: allTrips){
                current = new JSONObject();
                current.put("passenger",doc.get("passenger"));
                ...
                if(doc.get("endTime") != null){
                    ...
                    current.put("driverPayout", doc.get("driverPayout"));
